...
typed provider package. One provider is pinned to each app instance. Provider
discovery is cached, while live Agents and clients are never cached.
...
schedules a hidden activity with a deterministic, JSON-only payload. All file,
client, Agent, model, and tool I/O occurs in that activity, never in the
orchestrator.
