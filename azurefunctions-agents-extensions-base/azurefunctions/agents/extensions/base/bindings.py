...
_APP_STATES: weakref.WeakKeyDictionary[object, _AppState] = (
...
def _configured_state(app: object) -> _AppState:
    with _APP_STATES_LOCK:
        state = _APP_STATES.get(app)
    if state is None:
        raise RuntimeError("Agent app is not configured with a provider")
    return state
...
def markdown_agent(
    ...
    **provider_options: object,
    ...
    if "app_root" in provider_options:
        raise TypeError(
            "markdown_agent app_root is app-scoped; configure it on AgentFunctionApp"
        )
