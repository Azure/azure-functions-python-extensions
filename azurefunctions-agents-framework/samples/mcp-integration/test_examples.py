...
def check_imports(file_path: Path) -> List[str]:
    ...
    try:
        ...
        if 'sse_server.py' in str(file_path):
            # SSE server uses MCP FastMCP
            critical_imports = ['mcp']
        else:
            # Agent files need Azure Functions and agent framework
            critical_imports = ['azure.functions', 'azurefunctions.agents']
        missing_critical = []
