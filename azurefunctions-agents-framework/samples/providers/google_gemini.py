...
if __name__ == "__main__":
    ...
    print(f"Model: {agent.llm_config.model_name if agent.llm_config else 'No LLM configured'}")
    print(f"Provider: {agent.llm_config.provider.value if agent.llm_config else 'No provider'}")
    ...
    print('  -d \'{"message": "Write a creative story about a robot learning to paint."}\'\n')
    ...
    print('curl -X POST http://localhost:7071/api/gemini_assistant/chat \\')
    print('  -H "Content-Type: application/json" \\')
    print('  -d \'{"message": "Generate an image prompt for a sunset over mountains."}\'')
