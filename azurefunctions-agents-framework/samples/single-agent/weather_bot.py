...
async def retry_async_operation(operation, max_retries: int = 3, delay: float = 1.0):
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            logger.warning(f"Operation failed (attempt {attempt + 1}/{max_retries + 1}): {str(e)}. Retrying in {wait_time} seconds...")
...
async def get_lat_lng(location: str) -> Tuple[float, float]:
    ...
    if not OPENWEATHER_API_KEY:
        raise WeatherAPIError("OpenWeather API key not configured. Please set OPENWEATHER_API_KEY environment variable.")
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(url, params=params) as response:
                if response.status != 200:
                    raise WeatherAPIError(f"Geocoding API returned status {response.status}")
...
async def fetch_weather_data(lat: float, lng: float, units: str = "metric") -> Dict[str, Any]:
    ...
    async def _fetch_weather():
        async with aiohttp.ClientSession() as session:
            ...
            params = {"lat": lat, "lon": lng, "appid": OPENWEATHER_API_KEY, "units": units}
            
            async with session.get(url, params=params) as response:
                if response.status != 200:
                    raise WeatherAPIError(f"Weather API returned status {response.status}")
...
async def get_current_weather(location: str, units: str = "celsius") -> Dict[str, Any]:
    ...
    try:
        ...
        return {"error": str(e), "suggestion": "Please check the spelling or try a more specific location"}
    except WeatherAPIError as e:
        return {"error": f"Weather service error: {str(e)}", "suggestion": "Please try again later"}
    except Exception as e:
        logger.error(f"Unexpected error getting weather for {location}: {str(e)}")
        return {"error": f"Failed to get weather data: {str(e)}", "suggestion": "Please try again later"}
...
def get_weather_advice(condition: str, temperature: float = None, activity: str = None) -> Dict[str, Any]:
    ...
    base_advice = advice_map.get(condition, {
    ...
    return {
        ...
        "temperature_advice": temp_advice if temp_advice else "Temperature information not provided",
        "overall_recommendation": f"Current conditions are {condition} - " + base_advice["precautions"],
    }
