...
async def search_flights(
    ...
    result = {
        ...
        "recommendations": {"cheapest": flights[1], "fastest": flights[0]},
    }
...
async def get_destination_weather(location: str, travel_date: str) -> Dict[str, Any]:
    ...
    weather_data = {
        ...
        "temperature": {"high": 25, "low": 18, "unit": "celsius"},
...
async def find_restaurants(
    location: str, cuisine_type: str = "local", budget: str = "medium"
) -> Dict[str, Any]:
...
""",
    tools=[search_hotels],
    llm_config=llm_config,
    handoff_config=HandoffConfig(mode=HandoffMode.COORDINATOR, targets=[]),
)
...
async def travel_coordinator_demo(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        logging.info(
            f"Coordinator demo: {origin} → {destination}, {departure_date} to {return_date}"
        )
        ...
        travel_plan["flights"] = (
            flight_response.response
            if flight_response and flight_response.response
            else {}
        )
        ...
        travel_plan["hotels"] = (
            ...
            if hotel_response and hotel_response.response
            else {}
        )
        ...
        weather_response = await coordinator_runner.handoff_to(
            ...
            input_data={"location": destination, "travel_date": departure_date},
            conversation_id=conversation_id,
        ...
        travel_plan["restaurants"] = (
            restaurant_response.response
            if restaurant_response and restaurant_response.response
            else {}
        )
        ...
        consolidated_plan = {
            ...
            "recommendations": {
                "recommended_flight": travel_plan.get("flights", {})
                .get("recommendations", {})
                .get("cheapest"),
                "recommended_hotel": travel_plan.get("hotels", {})
                .get("recommendations", {})
                .get("best_value"),
                "weather_advice": travel_plan.get("weather", {}).get(
                    "recommendations", {}
                ),
                "must_try_restaurant": travel_plan.get("restaurants", {})
                .get("recommendations", {})
                .get("most_authentic"),
            },
            "estimated_total_cost": {
                "flight": travel_plan.get("flights", {}).get("best_price", 0)
                * 2,  # Round trip
                "hotel": travel_plan.get("hotels", {})
                .get("price_range", {})
                .get("min", 0)
                * 7,  # 7 nights
