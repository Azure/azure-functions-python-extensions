...
@app.route(
    route="custom/weather/chat", auth_level=func.AuthLevel.FUNCTION, methods=["POST"]
)
async def custom_weather_chat(req: HttpRequest) -> HttpResponse:
    ...
    try:
        ...
        session_id = req.headers.get(
            "X-Session-ID", f"session-{datetime.utcnow().timestamp()}"
        )
...
@app.service_bus_queue_trigger(
    arg_name="msg", queue_name="weather-requests", connection="ServiceBusConnection"
)
async def process_weather_queue_message(msg: func.ServiceBusMessage):
    ...
    try:
        ...
        response.to_dict()
        logger.info(
            f"Processed queue message {msg.message_id}: Status={response.status}"
        )
...
@app.timer_trigger(
    arg_name="timer", schedule="0 0 8 * * *", run_on_startup=False  # Daily at 8 AM
)
    ...
    try:
        ...
        request_data = {
            ...
            "context": {
                ...
                "scheduled_time": (
                    timer.schedule_status.get("last") if timer.schedule_status else None
                ),
        ...
        logger.info(
            f"Daily weather summary generated: {response.get('response', 'No summary')}"
        )
...
@app.blob_trigger(
    arg_name="blob", path="weather-data/{name}", connection="AzureWebJobsStorage"
)
async def process_weather_data_file(blob: func.InputStream):
    ...
    try:
        ...
        logger.info(
            f"Weather data analysis for {blob.name}: {response.get('response', 'No analysis')}"
        )
...
@app.route(
    route="weather/safe-chat", auth_level=func.AuthLevel.FUNCTION, methods=["POST"]
)
async def safe_weather_chat(req: HttpRequest) -> HttpResponse:
    ...
    try:
        ...
        return weather_runner.to_http_response(
            {"error": "Invalid request format", "message": str(e)}, status_code=400
        )
    except Exception as e:
