...
from .types import HandoffContext, HandoffMode, HandoffRequest
...
class ControlFlowManager:
    ...
    def validate_handoff(
        self, conversation_id: str, handoff_request: HandoffRequest
    ...
    def get_conversation_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_handoffs": sum(
                ctx.handoff_count for ctx in self._conversations.values()
            ),
            "longest_stack": max(
                (len(ctx.call_stack) for ctx in self._conversations.values()), default=0
            ),
