...
class TestHandoffTypes:
    ...
    def test_handoff_result_success(self):
        ...
        response_data = {"message": "Success", "agent": "TargetAgent", "metadata": {}}

        result = HandoffResult(
            success=True, target_agent="TargetAgent", response=response_data, error=None
        )
    ...
    def test_handoff_config_creation(self):
        ...
        config = HandoffConfig(
            mode=HandoffType.SWARM, strategy=HandoffStrategy.DIRECT, targets=[target]
        )
    ...
    def test_handoff_context_creation(self):
        ...
        context = HandoffContext(
            conversation_id="test-123", shared_context={"user_id": "user_456"}
        )
...
class TestHandoffIntegration:
    ...
    def test_handoff_config_with_multiple_targets(self):
        ...
        targets = [
            HandoffTarget(
                agent_name="TechnicalSupport", condition="priority == 'high'"
            ),
            HandoffTarget(
                agent_name="GeneralSupport", condition="priority == 'normal'"
            ),
        ]
