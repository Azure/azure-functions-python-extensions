...
class TestMCPServer:
    ...
    def test_mcp_server_stdio_params(self):
        ...
        params = MCPServerStdioParams(
            command="python", args=["-m", "mcp_server"], env={"MCP_DEBUG": "1"}
        )
    ...
    def test_mcp_server_stdio_initialization(self):
        ...
        params = MCPServerStdioParams(command="python", args=["-m", "weather_server"])

        server = MCPServer(
            name="weather-server", mode=MCPServerMode.STDIO, params=params
        )
    ...
    def test_mcp_server_sse_initialization(self):
        ...
        server = MCPServer(name="web-tools", mode=MCPServerMode.SSE, params=params)
    ...
    def test_mcp_server_streamable_http_initialization(self):
        ...
        params = MCPServerStreamableHttpParams(url="http://localhost:8080/stream")

        server = MCPServer(
            name="api-tools", mode=MCPServerMode.STREAMABLE_HTTP, params=params
        )
    ...
    async def test_mcp_server_connect_stdio(self):
        ...
        params = MCPServerStdioParams(command="python", args=["-m", "test_server"])

        server = MCPServer(name="test-server", mode=MCPServerMode.STDIO, params=params)
    ...
    async def test_mcp_server_connect_sse(self):
        ...
        server = MCPServer(name="sse-server", mode=MCPServerMode.SSE, params=params)
    ...
    async def test_mcp_server_connect_streamable_http(self):
        ...
        server = MCPServer(
            name="http-server", mode=MCPServerMode.STREAMABLE_HTTP, params=params
        )
    ...
    async def test_mcp_server_connection_error_handling(self):
        ...
        params = MCPServerStdioParams(command="nonexistent_command", args=["--invalid"])

        server = MCPServer(
            name="invalid-server", mode=MCPServerMode.STDIO, params=params
        )
...
class TestMCPUtil:
    ...
    async def test_mcp_util_connect_all_servers(self):
        ...
        for i in range(3):
            params = MCPServerStdioParams(command="python", args=["-m", f"server_{i}"])

            server = MCPServer(
                name=f"server-{i}", mode=MCPServerMode.STDIO, params=params
            )
    ...
    async def test_mcp_util_list_all_tools(self):
        ...
        server2.list_tools = AsyncMock(
            return_value=[Mock(name="search_web", description="Search the web")]
        )
    ...
    async def test_mcp_util_call_tool_server_not_found(self):
        ...
        with pytest.raises(ValueError, match="Server .* not found"):
            await util.call_tool_on_server("non-existent-server", "test_tool", {})
...
class TestMCPResultFormatter:
    ...
    def test_mcp_result_formatter_text_content(self):
        ...
        mock_content = [Mock(type="text", text="Weather: 72°F, sunny")]
    ...
    def test_mcp_result_formatter_error_content(self):
        ...
        mock_content = [Mock(type="text", text="Error: Unable to fetch weather data")]
...
class TestMCPIntegration:
    ...
    async def test_agent_with_mcp_servers(self):
        ...
        mcp_servers = [
            MCPServer(
                ...
                mode=MCPServerMode.STDIO,
                params=MCPServerStdioParams(
                    command="python", args=["-m", "weather_server"]
                ),
            ),
            MCPServer(
                name="web-server",
    ...
    async def test_mcp_tool_discovery_and_execution(self):
        ...
        server.call_tool = AsyncMock(
            return_value=Mock(content=[Mock(type="text", text="Result: 42")])
        )
