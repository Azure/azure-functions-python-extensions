...
class GraphClientReceiveMessage(SdkType, AzureGraphClientReceiveMessage):
    ...
    def get_sdk_type(
        self
    ) -> List[AzureGraphClientReceiveMessage]:
        """
        Uses the from_json method to parse the JSON payload into a list of
        GraphClientReceiveMessage objects.

        Returns:
            List of GraphClientReceiveMessage objects parsed from the JSON
            payload.
        """
        ...
        try:
            ...
            messages = AzureGraphClientReceiveMessage.from_json(
                self._json_payload
            )
            return messages
